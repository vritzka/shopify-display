...
static bool IRAM_ATTR request_cb(gptimer_handle_t timer,
...
void vCreateRequestTask(void *pvParameters) {
  ...
  ESP_ERROR_CHECK(
      gptimer_register_event_callbacks(requestTimerHandle, &cbs, NULL));
...
void vStartRequestTask(void) {
  ...
  xTaskCreatePinnedToCore(vCreateRequestTask, "SENSORTASK", 4096,
                          &ucParameterToPass, 10, &xRequestTaskHandle, 1);
